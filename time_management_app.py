...
def start_activity():
    ...
    with open(f'{st.session_state.activity_start.date()}.txt','a', encoding='utf-8') as f:
        f.write(f'{st.session_state.activity_start.strftime("%H:%M:%S")}, "Start", {st.session_state.activity_name} \n')

def show_current_activity():
    if st.session_state.activity_name != "":
        st.write(f"🚀 {st.session_state.activity_name} started at {st.session_state.activity_start.strftime('%d/%m/%Y, %H:%M:%S')} ! ")

def end_activity():
    with open(f'{st.session_state.activity_start.date()}.txt','a', encoding='utf-8') as f:
        f.write(f'{datetime.datetime.now().strftime("%H:%M:%S")}, "End", {st.session_state.activity_name} \n')
    st.session_state.activity_in_progress = False
    st.write(f"✨ {st.session_state.activity_name} ended at {datetime.datetime.now().strftime('%d/%m/%Y, %H:%M:%S')} ! ")
...
def page2():
    try :
        data = pd.read_csv(f'{st.session_state.activity_start.date()}.txt', header=None, names=['Time', 'Type', 'Name of activity'])
        st.dataframe(data)
    except:
        st.header(f"🤭 No data available yet, could find the file : {datetime.datetime.now().date()}.txt !")
